...
if start_button:
    ...
    # 📌 방어막 1: 빈 장부를 미리 만들어 두기 (서버가 터져도 에러 안 나게)
    ...
    try:
        ...
        # 📌 방어막 2: 재무 데이터 요청 (실패 시 경고만 띄우고 패스)
        try:
        ...
        # 📌 방어막 3: 수급 데이터 요청 (실패 시 경고만 띄우고 패스)
        try:
        ...
        for i, (index, row) in enumerate(target_stocks.iterrows()):
            ...
            try:
                ...
                # 재무 채점 (서버가 터져서 fund_df가 비어있으면 자연스럽게 0점 처리됨)
                ...
                # 수급 채점 (서버가 터져서 비어있으면 자연스럽게 0점 처리됨)
                ...
                # 차트 채점 (FinanceDataReader는 절대 안 터짐)
                try:
        ...
        # 혹시라도 프로그램 자체가 터지는 것을 방지하는 최후의 보루
        st.error("알 수 없는 에러가 발생했습니다. 잠시 후 다시 실행해 주세요.")
else:
